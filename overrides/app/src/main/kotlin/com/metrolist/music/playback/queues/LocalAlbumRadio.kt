...
class LocalAlbumRadio(
    ...
    override suspend fun getInitialStatus(): Queue.Status = withContext(IO) {
        Queue.Status(
            ...
            items = albumWithSongs.songs.map { it.toMediaItem() }.filterAllowedMediaItems(),
            mediaItemIndex = com.metrolist.innertube.utils.selectedIndex(
                albumWithSongs.songs.map { it.toMediaItem() }.filterAllowedMediaItems(),
                albumWithSongs.songs.getOrNull(startIndex)?.id,
            ) { it.mediaId }
        )
    }
    ...
    override suspend fun nextPage(): List<MediaItem> = withContext(IO) {
        if (!firstTimeLoaded) {
            ...
            return@withContext nextResult.items.filterNot { item -> albumWithSongs.songs.any { it.id == item.id } }.filterAllowedArtists().map { it.toMediaItem() }
        }
        val nextResult = YouTube.next(endpoint, continuation).getOrThrow()
        continuation = nextResult.continuation
