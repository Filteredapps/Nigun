...
@Composable
private fun displaySearchSummaryTitle(title: String): String {
    val normalized = title.trim().lowercase()
    return when {
        normalized == "albums" || normalized == "אלבומים" -> stringResource(R.string.filter_releases_versions)
        normalized == "playlists" || normalized == "פלייליסטים" -> stringResource(R.string.filter_artist_playlists)
        normalized == "songs" || normalized == "שירים" -> stringResource(R.string.filter_songs)
        normalized == "artists" || normalized == "אמנים" -> stringResource(R.string.filter_artists)
        else -> title
    }
}
...
fun OnlineSearchResult(
    ...
    LaunchedEffect(lazyListState, searchFilter, itemsPage?.continuation) {
        snapshotFlow {
    ...
    Column(
        ...
        Box(modifier = Modifier.weight(1f)) {
            Column(
                ...
                val visibleChips =
                    listOf(
                        ...
                        FILTER_ALBUM to stringResource(R.string.filter_releases_versions),
                        FILTER_ARTIST to stringResource(R.string.filter_artists),
                        FILTER_FEATURED_PLAYLIST to stringResource(R.string.filter_artist_playlists),
                    )
                ...
                LazyColumn(
                    ...
                    if (searchFilter == null) {
                        searchSummary?.summaries?.forEach { summary ->
                            item {
                                NavigationTitle(displaySearchSummaryTitle(summary.title))
                            }
                        ...
                        if (itemsPage?.continuation != null) {
                            if (searchFilter?.value?.let { viewModel.paginationPaused[it] } == true) {
                                item(key = "load_more_paused") {
                                    androidx.compose.material3.TextButton(
                                        onClick = { viewModel.loadMore(manual = true) },
                                        modifier = Modifier.fillMaxWidth(),
                                    ) { Text(stringResource(R.string.nigun_load_more)) }
                                }
                            } else {
                                item(key = "loading") {
                                    ShimmerHost {
            ...
            // Autocomplete/search suggestion overlay is disabled here so focusing the search bar
            // does not hide the current results behind an empty suggestions sheet. The user can
            // edit the query and press Search to run a new search.
            HideOnScrollFAB(
