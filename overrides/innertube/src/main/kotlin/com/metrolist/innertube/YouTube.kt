...
object YouTube {
    ...
    suspend fun album(
        ...
        val rawAlbum = AlbumItem(
            browseId = browseId,
            playlistId = requireNotNull(AlbumPage.getPlaylistId(response)) { "Album playlist is unavailable" },
            title = requireNotNull(AlbumPage.getTitle(response)) { "Album title is unavailable" },
            artists = AlbumPage.getArtists(response),
            year = AlbumPage.getYear(response),
            thumbnail = AlbumPage.getThumbnail(response).orEmpty(),
        )
        val albumItem = resolveArtistIds(listOf(rawAlbum)).first()
        val songs = if (withSongs) {
            val embedded = AlbumPage.getSongs(response, albumItem)
            if (browseId.contains("FEmusic_library_privately_owned_release_detail")) {
                embedded
            } else {
                val result = albumSongs(albumItem.playlistId, albumItem)
                result.getOrElse { error -> embedded.ifEmpty { throw error } }.ifEmpty { embedded }
            }
        } else emptyList()
        val resolvedSongs = resolveArtistIds(songs)
        // Compilation albums can contain an allowed collaborator even if their header does not.
        val creditedArtists = (albumItem.artists.orEmpty() + resolvedSongs.flatMap { it.artists })
            .distinctBy { it.id to it.name }
        AlbumPage(
            album = albumItem.copy(artists = creditedArtists),
            songs = resolvedSongs,
            otherVersions = response.contents?.twoColumnBrowseResultsRenderer?.secondaryContents
                ?.sectionListRenderer?.contents.orEmpty()
                .flatMap { it.musicCarouselShelfRenderer?.contents.orEmpty() }
    ...
    private val artistIdResolver = com.metrolist.innertube.utils.ArtistIdResolver { name ->
        search(name, SearchFilter.FILTER_ARTIST).getOrNull()?.items
            ?.filterIsInstance<ArtistItem>()
            ?.filter { it.title.trim().equals(name, ignoreCase = true) }
            ?.map { it.id }?.distinct()?.singleOrNull()
    }

    suspend fun <T : YTItem> resolveArtistIds(items: List<T>): List<T> =
        artistIdResolver.resolve(items)
}
